...
namespace CIL {
    ...
    class ImageInfo
    {
        ...
        std::string m_internal_info_image_type = "NULL";
      ...
      public:
        ...
        ImageInfo(uint32_t width, uint32_t height, uint32_t num_components,
                  ...
                  const std::string& internal_info_image_type = "NULL");
    ...
    template <typename T> void RegisterImageHandler()
    {
        std::unique_ptr<ImageHandler> handler(new T());
        AddImageHandler(std::move(handler));
    }
} // namespace CIL
#endif
